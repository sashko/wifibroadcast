...
class RadiotapHeaderTx {
 public:
  ...
  explicit RadiotapHeaderTx(const UserSelectableParams &params) {
    if (params.mcs_index < radiotap::tx::MCS_MIN || params.mcs_index > radiotap::tx::MCS_MAX) {
    ...
    radiotapHeaderData.presence =
        radiotap::tx::writePresenceBitfield({IEEE80211_RADIOTAP_TX_FLAGS, IEEE80211_RADIOTAP_MCS});
...
static_assert(sizeof(RadiotapHeaderTx) == RadiotapHeaderTx::SIZE_BYTES, "ALWAYS TRUE");
static_assert(sizeof(RadiotapHeaderWithTxFlagsAndMCS) ==
                  RadiotapHeaderTx::SIZE_BYTES, "ALWAYS TRUE");

namespace RadiotapHelper {

// [RadiotapHeaderTx | Ieee80211HeaderRaw | customHeader (if not size 0) | payload (if not size 0)]
static std::vector<uint8_t> create_radiotap_wifi_packet(const RadiotapHeaderTx & radiotapHeader,
