...
static std::optional<ParsedRxRadiotapPacket> process_received_radiotap_packet(const uint8_t *pkt,const int pkt_len) {
  ...
  // AND the driver reports signal (noise,lock) if given for the adapter and rf paths(s) alltogether
  // Seems to be the case for all drivers, definitely for the openhd supported ones.
  ...
  return ParsedRxRadiotapPacket{ieee80211Header, payload, payloadSize, has_radiotap_f_bad_fcs,adapter,rf_paths};
}


static std::string key_rf_indicators_to_string(const KeyRfIndicators& indicators){
  std::stringstream ss;
  ss<<"{";
...
static std::string all_rf_path_to_string(const std::vector<KeyRfIndicators>& all_rf_path){
  std::stringstream ss;
  ss<<"RF Paths:";
