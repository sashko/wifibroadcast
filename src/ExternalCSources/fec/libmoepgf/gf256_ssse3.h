...
xorr_sse2(uint8_t *region1, const uint8_t *region2, size_t length)
{
    ...
    for (end=region1+length; region1<end; region1+=16, region2+=16) {
        in  = _mm_load_si128((const __m128i *)region2);
        out = _mm_load_si128((const __m128i *)region1);
        out = _mm_xor_si128(in, out);
        _mm_store_si128((__m128i *)region1, out);
    }
}
                        ...
                        uint8_t constant, size_t length)
{
    ...
    for (end=region1+length; region1<end; region1+=16, region2+=16) {
        in2 = _mm_load_si128((const __m128i *)region2);
        in1 = _mm_load_si128((const __m128i *)region1);
...
mulrc256_shuffle_ssse3(uint8_t *region, uint8_t constant, size_t length)
{
    ...
    for (end=region+length; region<end; region+=16) {
        in = _mm_load_si128((const __m128i *)region);
        ...
        h = _mm_shuffle_epi8(t2, h);
        out = _mm_xor_si128(h, l);
        _mm_store_si128((__m128i *)region, out);
    }
}
