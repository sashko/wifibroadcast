...
static void test_send_and_receive() {
  ...
  SocketHelper::UDPReceiver receiver(
  ...
  // SocketHelper::UDPForwarder
  // forwarder(SocketHelper::ADDRESS_LOCALHOST,XPORT);
  ...
  std::cout << "N sent bytes:" << nForwardedBytes
            << " Received:" << nReceivedBytes << "\n";
  if (nForwardedBytes != nReceivedBytes) {
    throw std::runtime_error("Dropped packets or impl bugged\n");
  }
}

// Here we have a simple test for adding and removing new forwarding IP
// addresses. (UDPMultiForwarder)
