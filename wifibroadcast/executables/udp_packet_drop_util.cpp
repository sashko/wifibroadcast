...
static void on_new_udp_data(const uint8_t *payload,
...
int main(int argc, char *const *argv) {
  udp_forwarder =
      std::make_unique<SocketHelper::UDPForwarder>("127.0.0.1", 5600);
  packet_drop_emulator = std::make_unique<PacketDropEmulator>(0);

  udp_receiver = std::make_unique<SocketHelper::UDPReceiver>(
      "127.0.0.1", 5599,
