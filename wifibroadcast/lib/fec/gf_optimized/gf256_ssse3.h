...
// AVX2 'could' beat ssse3, but for my personal use case this difference didn't
// justify making the optimization even more complex Regarding alignment: I
// modified the code to use "u" (unaligned) instructions everyhwere, so
// alignment doesn't matter anymore It used to be different in the original
// impl.
...
void maddrc256_shuffle_ssse3(uint8_t *region1, const uint8_t *region2,
...
void mulrc256_shuffle_ssse3(uint8_t *region1, const uint8_t *region2,
                            uint8_t constant, size_t length) {
  assert(length % 16 == 0);
  uint8_t *end;
