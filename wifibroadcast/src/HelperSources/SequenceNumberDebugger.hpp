...
/**
 * Debug the n lost packets and the n of packet gaps by for a continuous stream
 * of packets with increasing sequence number.
 */
class SequenceNumberDebugger {
 public:
  SequenceNumberDebugger() { gapsBetweenLostPackets.reserve(1000); }
  /**
  ...
  void sequenceNumber(const int64_t seqNr) {
    ...
    if (delta <= 0) {
      wifibroadcast::log::get_default()->debug(
          "got packet nr: {} after packet nr: {}", seqNr,
          lastReceivedSequenceNr);
      return;
    }
    if (delta > 1) {
  ...
  void debug(bool clear) {
    ...
    ss << "N packets received:" << nReceivedPackets << "\tlost:" << nLostPackets
       << "\n";
    ss << "Packet gaps:"
       << StringHelper::vectorAsString(gapsBetweenLostPackets);
