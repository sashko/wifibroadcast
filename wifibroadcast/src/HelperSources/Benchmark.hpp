...
class PacketizedBenchmark {
 public:
   ...
   * @param factor1 use a factor other than 1.0 if the packet size changes
   * during the benchmarked step
   */
  explicit PacketizedBenchmark(std::string name1, double factor1 = 1.0f)
      : name(std::move(name1)), factor(factor1){};
  void begin() {
  ...
  void doneWithPacket(const double packetSizeBytes) {
    ...
    if (delta > std::chrono::seconds(1)) {
      ...
      std::cout << "curr. Packets per second:" << currPacketsPerSecond
                << " before " << name << ": " << currBitRate_MBits << "Mbit/s";
      if (factor != 1.0f) {
        std::cout << " after " << name << ": " << currBitRate_MBits * factor
                  << "MBit/s";
      }
      std::cout << "\n";
      logTs = std::chrono::steady_clock::now();
  ...
  void end() {
    ...
    const float testDurationSeconds =
        std::chrono::duration_cast<std::chrono::milliseconds>(testDuration)
            .count() /
        1000.0f;
    // std::cout<<"Wanted duration:"<<options.benchmarkTimeSeconds<<" actual
    // duration:"<<testDurationSeconds<<"\n";

    double totalPacketsPerSecond =
        totalPacketsDelta / (double)testDurationSeconds;
    ...
    if (factor != 1.0f) {
      std::cout << " after " << name << ": " << totalBitRate_MBits * factor
                << "MBit/s";
    }
    std::cout << "\n";
  }
 ...
 private:
  std::chrono::steady_clock::time_point testBegin =
      std::chrono::steady_clock::now();
  std::chrono::steady_clock::time_point logTs =
      std::chrono::steady_clock::now();
  int packetsDelta = 0;
  int totalPacketsDelta = 0;
  //
...
class DurationBenchmark {
 public:
  DurationBenchmark(std::string name1, int dataSizeBytes1)
      : name(name1), dataSizeBytes(dataSizeBytes1) {}
  void start() { before = std::chrono::steady_clock::now(); }
  void stop() {
    const auto delta = std::chrono::steady_clock::now() - before;
    const auto deltaUs =
        std::chrono::duration_cast<std::chrono::microseconds>(delta).count();
    // std::cout<<"Encoding a block of
    // size:"<<StringHelper::memorySizeReadable(blockSizeBytes)<<
  ...
  void print() {
    ...
    std::cout << "This would equate to a throughput of: "
              << emulatedThroughputMBits << " Mbit/s\n";
  }
