...
/**
 * Create @param nBuffers buffers of size @param bufferSize filled with
 * semi-random data. Each of the generated buffers contains different data than
 * the previous generated one (by a almost 100% chance)
 */
static std::vector<std::shared_ptr<std::vector<uint8_t>>>
createSemiRandomBuffers(const std::size_t nBuffers,
  ...
  for (auto &buffer : ret) {
    ...
    // NOTE: I think this one doesn't work since the mt19937 is copied with each
    // invocation ?!
...
template <size_t S>
static std::vector<std::array<uint8_t, S>> createSemiRandomBuffers2(
...
class RandomBufferPot {
 public:
  ...
  RandomBufferPot(const std::size_t nBuffers, const std::size_t bufferSize) {
    m_buffers =
        SemiRandomBuffers::createSemiRandomBuffers(nBuffers, bufferSize);
  }
  // get a semi-random data buffer for this sequence number. If the sequence
  // number is higher than the n of allocated buffers, it loops around. As long
  // as this pot is big enough, it should be sufficient to emulate a random data
  // stream
