...
// 2) ssse3 is already "fast enough" and supported on pretty much any platform
// (other than AVX2) If you want to, add and test the code here ;)
...
void maddrc256_shuffle_avx2(uint8_t *region1, const uint8_t *region2,
                            uint8_t constant, size_t length) {
  assert(length % 32 == 0);
  // std::cout<<"maddrc256_shuffle_avx2:"<<length<<"
...
void mulrc256_shuffle_avx2(uint8_t *region1, const uint8_t *region2,
                           uint8_t constant, size_t length) {
  assert(length % 32 == 0);
  // assert(is_aligned(region1,32));
  // assert(is_aligned(region2,32));
