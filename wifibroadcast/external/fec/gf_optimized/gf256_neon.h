...
// Regrading alignment:
// https://developer.arm.com/documentation/ddi0344/f/Cihejdic I think neon by
// default doesn't care about alignment, only if the alignment is explicitly
// specified it needs to match
...
void xorr_neon_64(uint8_t *region1, const uint8_t *region2, size_t length) {
  // std::cout<<"Xx neon"<<length<<"
  // "<<(int)region1[0]<<":"<<(int)region2[0]<<"\n";
  assert(length % 8 == 0);
  uint8_t *end;
  uint8x8_t in, out;
...
static void maddrc256_shuffle_neon_64(uint8_t *region1, const uint8_t *region2,
...
void mulrc256_shuffle_neon_64(uint8_t *region1, const uint8_t *region2,
                              uint8_t constant, size_t length) {
  assert(length % 8 == 0);
  uint8_t *end;
  uint8x8x2_t t1, t2;
