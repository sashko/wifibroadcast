...
// This code exists to validate the (optimized) gf256 math operations at run
// time
...
// return x + a*b
gal8 gal_madd(gal8 x, gal8 a, gal8 b) { return gal_add(x, gal_mul(a, b)); }

// same as above, but for memory regions ( to test optimized versions)
static void gal_mul_region(gf *dst, const gf *src, gf c, const int sz) {
  for (int i = 0; i < sz; i++) {
    dst[i] = gal_mul(src[i], c);
  }
}
static void gal_madd_region(gf *dst, const gf *src, gf c, const int sz) {
  for (int i = 0; i < sz; i++) {
