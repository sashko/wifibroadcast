...
1) variable block size: If you are transmitting h264,h265 or mjpeg video, automatically end a block
with the end of each frame. This reduces latency and can increase resiliency against packet loss
2) disable FEC: For telemetry or RC data, FEC doesn't really make sense. By disabling FEC you get
the same properties as "true UDP".
3) only set information about FEC (fixed,variable,disabled) on the tx: The tx and rx pipeline only have to match on the 
radio_port, the rest is done automatically
